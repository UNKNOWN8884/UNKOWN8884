...
async def start(client, message):
        ...
        btn = [
            [
                InlineKeyboardButton(
                    "ႮᏢᎠᎪͲᎬ ᏟᎻᎪΝΝᎬᏞ Ͳϴ ႮՏᎬ ᎷᎬ🔥", url=invite_link.invite_link
                )
            ]
        ]

        if message.command[1] != "subscribe":
            btn.append([InlineKeyboardButton(" 🔄 ͲᎡᎽ ᎪᏀᎪᏆΝ", callback_data=f"checksub#{message.command[1]}")])
    ...
    if len(message.command) ==2 and message.command[1] in ["subscribe", "error", "okay", "help"]:
        buttons = [[
            ...
            InlineKeyboardButton('ᎫϴᏆΝ ҒᏆᎡՏͲ ՏᎻϴᏔᎬᎡՏ 🎬 ҒϴᎡ ᎷϴᎡᎬ  ᎷϴᏙᏆᎬՏ📡 , url=https://t.me/firstshowers')
            ],
        reply_markup = InlineKeyboardMarkup(buttons)
        await message.reply_photo(
            photo=random.choice(PICS),
